...
describe("/books", () => {
  ...
  describe("GET /", () => {
    ...
    describe("GET /search", () => {
      ...
      it("should return multiple matching books sorted by best multiple terms", async () => {
        const searchTerm = 'Fantasy and Kings'
        const res = await request(server).get("/books/search?query=" + encodeURI(searchTerm));
        expect(res.statusCode).toEqual(200);
        expect(res.body).toHaveLength(5);
        expect(res.body[0]).toMatchObject(testBooks.find(book => book.title === 'The Way of Kings'));
        expect(res.body[4]).toMatchObject(testBooks.find(book => book.title === 'Steelheart'));
        expect(res.body).toContainEqual(expect.objectContaining(testBooks.find(book => book.title === 'The Fifth Season')));
        expect(res.body).toContainEqual(expect.objectContaining(testBooks.find(book => book.title === 'The Obelisk Gate')));
        expect(res.body).toContainEqual(expect.objectContaining(testBooks.find(book => book.title === 'The Stone Sky')));
      });
    });

    describe("/authors/stats", () => {
      it("should return stats grouped by authorId with sorted titles", async () => {
        // Note, you may want to look at this for some ideas on title sorting:
        // https://www.mongodb.com/docs/manual/reference/operator/aggregation/sortArray/#examples
        ...
        expect(res.body).toContainEqual({
          ...
          numBooks: 2,
          titles: [testBooks[1].title, testBooks[0].title]
        });
        expect(res.body).toContainEqual({
          authorId: savedAuthors[1]._id,
          averagePageCount: 211 + 1/3,
          numBooks: 3,
          titles: [testBooks[2].title, testBooks[4].title, testBooks[3].title]
        });
        expect(res.body).toContainEqual({
