...
This week is an exercise in authentication, authorization, and middleware all combined together.
...
- have used middleware for authentication, authorization, and error handling
...
1. Make sure you have a recent version of [Node.js](https://nodejs.org/en/download/) installed on your computer. I am using Node v22, but anything above 18 will be fine.
...
We will need routes for a user to sign up, login, and change password. This will use a `User` model. Then we will need routes for items and orders, along with `Item` and `Order` models. The models are provided, but you may want to modify them.
